...
ISR(TIMER2_CTC_VEC)
{
	if (TIMER2_APfunCallBack[TIMER2_DELAY_CALLBACK] != NULL && ISR_u32Counter == TIMER2_u8NumberOfOverflow * TIMER2_u32StartDelay && (ISR_u32ActionCounter * TIMER2_u32StartDelay) <= TIMER2_u32StopTime)
	{
	...
	{
		ISR_u32Counter++;
		if ( (!(ISR_u32Counter % TIMER2_u8NumberOfOverflow)) && TIMER2_u8Preload )
		{
			TIMER2_VIOD_SET_PRELOAD(TIMER2_u8Preload);
		}
	}
...
ISR(TIMER2_OVF_VEC)
{
	...
	{
		ISR_u32Counter++;
		if ( ((ISR_u32Counter % TIMER2_u8NumberOfOverflow) ==0 ) && TIMER2_u8Preload !=0 )
		{
			TIMER2_VIOD_SET_PRELOAD(TIMER2_u8Preload);
		}
	}
